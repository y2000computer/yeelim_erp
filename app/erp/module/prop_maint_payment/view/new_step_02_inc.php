					...
					<div class="sidebarContentCol" id="TransactionsDiv">
						...
						<table class="searchResult" border="0" cellspacing="0" cellpadding="0">
							<tbody>
								...
								<th style="text-align:left" width ="">Building</th>
								<th style="text-align:left" width ="8%">Invoice No.</th>
								<th style="text-align:left" width ="9%">Invoice Date</th>
								...
								if ($arr_count_general_model >0){
									...
									foreach ($arr_general_model as $general_model): 
										...
										echo '<td>';
										echo '<a href="'.actionURL('new_step_03','?item_id='.$general_model['inv_id'].'&lot_id='.$lot_id.'&page='.$page).'">'.htmlspecialchars($general_model['inv_code']).'&nbsp;'.'&raquo;'.'</a>';
										echo '</td>';

										echo '<td>'.htmlspecialchars(YMDtoDMY($general_model['inv_date'])).'</td>';						
										...
										echo '<td style="text-align:right" >'.htmlspecialchars(number_format($general_model['balance'],2)).'</td>';
